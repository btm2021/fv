...
(function (root, factory) {
  ...
  const Stat2BoxStrategyIndicator = {
    ...
    defaultStyle: {
      ...
      showGuideLines   : { type: 'checkbox', label: 'Show Solid TP/SL/Entry Lines', value: true },
      ...
      showFVG          : { type: 'checkbox', label: 'Show FVG Zones', value: true },
      showLiquidity    : { type: 'checkbox', label: 'Show Liquidity Lines (BSL/SSL)', value: true },
      showRibbon       : { type: 'checkbox', label: 'Show ATRBot Ribbon Cloud', value: true },
      showTrail2       : { type: 'checkbox', label: 'Show Dynamic Trailing Stop', value: true },
      ...
      stopColor        : { type: 'color', label: 'Trailing Stop Color', value: '#a855f7' },
    ...
    calculate: function (candles, inputs) {
      ...
      // 2. ATRBot Calculation via SMC.atrBot
    ...
    renderCanvas: function (ctx, result, style, helpers) {
      ...
      if (style.showFVG !== false && fvgList && fvgList.length > 0) {
        ...
        for (let i = 0; i < fvgList.length; i++) {
          ...
          const fvgColor = isBull ? '#10b981' : '#f43f5e';

          ctx.fillStyle = hexToRgba(fvgColor, isMit ? 0.07 : 0.18);
      ...
      if (style.showLiquidity !== false && liqList && liqList.length > 0) {
        for (let i = 0; i < liqList.length; i++) {
          ...
          const baseColor = isBSL ? '#ec4899' : '#8b5cf6';
      ...
      if (atrData && atrData.length >= 2) {
        ...
        if (style.showRibbon !== false) {
          for (let i = 1; i < n; i++) {
            ...
            ctx.fillStyle = isBull ? (style.bullCloudColor ? hexToRgba(style.bullCloudColor, 0.12) : 'rgba(16, 185, 129, 0.12)')
                                   : (style.bearCloudColor ? hexToRgba(style.bearCloudColor, 0.12) : 'rgba(244, 63, 94, 0.12)');
            ctx.beginPath();
            ctx.moveTo(x1, y1_t1);
            ctx.lineTo(x2, y2_t1);
            ctx.lineTo(x2, y2_t2);
      ...
      if (style.showCards !== false && cards && cards.length > 0) {
        ...
        const cardW = Math.max(Number(style.cardWidth) || 230, 200);
        const cardH = 138;
        const cardBg = style.cardBackground || '#0b1120';
        const cardAlpha = Number(style.cardOpacity) || 0.95;
        ...
        for (let idx = 0; idx < visibleCards.length; idx++) {
          ...
          if (Math.abs(cardX - prevCardX) < cardW + 10) {
            offsetStagger = 35; // Stagger if horizontal overlap with previous card
          }
          prevCardX = cardX;
          ...
          // Record clickable box bounds for modal interaction
          ...
          // Compact Status Pill on Header Right (Short non-overlapping badge)
          ...
          const col1X = cardX + 12;                // Label Column (Left)
          const col2X = cardX + 64;                // Price Value Column (Left)
          const col3X = cardX + cardW - 12;        // % ROI / Risk Column (Right-Aligned)

          const r1Y = cardY + 45;                  // Row 1: Entry
          const r2Y = cardY + 67;                  // Row 2: TP1
          const r3Y = cardY + 89;                  // Row 3: TP2
          const r4Y = cardY + 111;                 // Row 4: SL
          const r5Y = cardY + 127;                 // Footer hint
          ...
          if (style.showGuideLines !== false) {
            ...
            // 1. ENTRY Solid Line (Cyan / White)
            const yEntry = getY(card.entryPrice);
            drawSolidPriceLine(yEntry, '#0284c7', '🔹 ENTRY', pEntry);

            // 2. TP1 Solid Line (Emerald Green)
            const yTp1 = getY(card.tp1Price);
            drawSolidPriceLine(yTp1, '#10b981', '🎯 TP1', pTp1, `(+${card.tp1Pct.toFixed(1)}%)`);

            // 3. TP2 Solid Line (Bright Sky Blue)
            const yTp2 = getY(card.tp2Price);
            drawSolidPriceLine(yTp2, '#06b6d4', '🏆 TP2', pTp2, `(+${card.tp2Pct.toFixed(1)}%)`);

            // 4. SL Solid Line (Vibrant Rose Red)
            const ySl = getY(card.slPrice);
            drawSolidPriceLine(ySl, '#f43f5e', '🛑 SL', pSl, `(-${card.slPct.toFixed(1)}%)`);
          }
        }
      }
